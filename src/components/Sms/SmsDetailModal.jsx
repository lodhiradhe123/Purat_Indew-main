...
import { gsmRead } from "../../services/api";
...
const SmsDetailsModal = ({
  ...
  const fetchBroadcastDetails = async () => {
    ...
    try {
      ...
      if (data.status === 1) {
        setData(data.data); // Set broadcast details on success
                ...
                <TableBody>
                  {paginatedBroadcasts.map((item) => (
                    <TableRow key={item.id} className={classes.tableRow}>
                      ...
                      <TableCell>{item.sender_id || "N/A"}</TableCell>
                      <TableCell>{item.mo_no || "N/A"}</TableCell>
